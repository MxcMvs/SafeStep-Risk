...
function normalizeRisk(r) {
  ...
  let name = 'Unnamed risk';

  if (r) {
    if (typeof r.name === 'string') name = r.name;
    else if (typeof r.risk === 'string') name = r.risk;
    else if (typeof r.name === 'object') name = r.name.name || JSON.stringify(r.name);
  }
...
function getInitialRisks() {
  ...
  saveJSON(STORAGE.rated, normalized);
  return normalized;
}
...
function drawChart() {
  ...
  chartInstance = new Chart(chartCanvas.getContext('2d'), {
    type: 'bar',
    data: { labels, datasets: [{ label: 'Risk Score', data, backgroundColor: bg }] },
    options: {
      responsive: true,
      maintainAspectRatio: false,
      plugins: {
        tooltip: { callbacks: { title: i => i[0]?.label || '', label: ctx => `Score: ${ctx.parsed.y}` } },
        legend: { display: true }
      },
      scales: {
...
function applyFilter(type) {
  if (type === 'alphabetical') risks.sort((a, b) => a.name.localeCompare(b.name));
  else if (type === 'score') risks.sort((a, b) => b.score - a.score);
  saveJSON(STORAGE.rated, risks);
  renderAll();
}
...
async function exportPdf() {
  ...
  const body = risks.map(r => [
    r.name, r.mitigation || '', String(r.likelihood), String(r.impact), String(r.score), formatDate(r.updatedAt)
  ]);

  doc.autoTable({ startY: 60, head, body, styles: { fontSize: 9, cellPadding: 4 }, headStyles: { fillColor: [51, 51, 51] } });
